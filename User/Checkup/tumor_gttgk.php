<!-- Tumor GTTGK ການກວດບົງຊີ້ຕ່ອມມະເຮັງ(Tumor_GTTGK)-->
<?php
  $title = "ການກວດບົງຊີ້ຕ່ອມມະເຮັງ";
  $path = "../../";
  $links = "../";
  $session_path = "../../";
  include ("../../header-footer/header.php");
?>
<form action="Tumor-Gttgk" method="POST" id="form_upload" enctype="multipart/form-data">
...
<form action="export/export_tumor_gttgk.php" method="POST" id="form_export" target="_blank">
    <div class="row">
...
<form action="Tumor-Gttgk" id="formDelete" method="POST" enctype="multipart/form-data">
 ...
 if(isset($_POST['btnDelete'])){
    if(isset($_POST["id_delete"])){
        foreach($_POST['id_delete'] as $id){
            $result_delete = mysqli_query($conn,"call del_tumor_gttgk('$id')");
        }
        if(!$result_delete){
            echo"<script>";
            echo"window.location.href='Tumor-Gttgk?delete2=fail';";
            echo"</script>";
        }
        else{
            echo"<script>";
            echo"window.location.href='Tumor-Gttgk?delete=success';";
            echo"</script>";
        }
    }
    else{
        echo"<script>";
        echo"window.location.href='Tumor-Gttgk?del=null';";
        echo"</script>";
    }
 }
    include ("../../header-footer/footer.php");
    if(isset($_POST["file_upload"])){
        $obj->import_tumor_gttgk($_FILES["checkup_file"]["tmp_name"],$_POST["year"],$user_id);
    }
