 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   ...
   $obj->select_se_limit("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%",$page);
}
else
{
   $obj->select_se_limit("%%", "%%", "%%", $page);
}
// $output .= '
// <style>
...
// </style>
// ';
if(mysqli_num_rows($result_se_limit) > 0)
{
 $output .= '
 ...
 ';
 $no_ = $rank;
 while($row = mysqli_fetch_array($result_se_limit))
 {
$no_ += 1;
  $output .= '
    <tr>
    <td><input type="checkbox" id="" name="id_delete[]" value="'.$row["se_id"].'"></td>
        <td>'.$no_.'</td>
        <td>'.$row["barcode"].'</td>
        <td>'.$row["emp_id"].'</td>
        <td>'.$row["emp_name"].'</td>
 ...
 mysqli_free_result($result_se_limit);  
 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   $obj->select_se("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%");
}
else
{
   $obj->select_se("%%", "%%", "%%");
