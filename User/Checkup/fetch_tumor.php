 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   ...
   $obj->select_tumor_marker_limit("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%",$page);
}
else
{
   $obj->select_tumor_marker_limit("%%", "%%", "%%", $page);
}
// $output .= '
// <style>
...
// </style>
// ';
if(mysqli_num_rows($result_tumor_marker_limit) > 0)
{
 $output .= '
 ...
 ';
 $no_ = $rank;
 while($row = mysqli_fetch_array($result_tumor_marker_limit))
 {
$no_ += 1;
  $output .= '
    <tr>
 ...
 mysqli_free_result($result_tumor_marker_limit);  
 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   $obj->select_tumor_marker("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%");
}
else
{
   $obj->select_tumor_marker("%%", "%%", "%%");
}
 $count = mysqli_num_rows($result_tumor_marker);
 mysqli_free_result($result_tumor_marker);  
 mysqli_next_result($conn);
 $a = ceil($count/100);
 if(isset($_POST['page'])){
    if($_POST['page'] > 1){
