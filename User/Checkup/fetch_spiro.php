 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   ...
   $obj->select_spiro_limit("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%",$page);
}
else
{
   $obj->select_spiro_limit("%%", "%%", "%%", $page);
}
// $output .= '
// <style>
...
if(mysqli_num_rows($result_spiro_limit) > 0)
{
 $output .= '
 ...
 <table class="table-bordered" style="width: 3000px;text-align: center;">
     <tr style="font-size: 18px;">
         ...
         <th style="width: 87px;">FVC(MEANS)</th>
         <th style="width: 50px;">FVC(PREDICT)</th>
         <th style="width: 87px;">FVC(%PREDICTS)</th>
         <th style="width: 50px;">FEV1(MEANS)</th>
         <th style="width: 87px;">FEV1(PREDICT)</th>
         <th style="width: 50px;">FEV1(%PREDICT)</th>
         <th style="width: 50px;">FEC1/FVC%</th>
         <th style="width: 87px;">Conclusion</th>
         <th style="width: 50px;">Remark</th>
     </tr>
 ';
 $no_ = $rank;
 while($row = mysqli_fetch_array($result_spiro_limit))
 {
$no_ += 1;
  $output .= '
    <tr>
 ...
 mysqli_free_result($result_spiro_limit);  
 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   $obj->select_spiro("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%");
}
else
{
   $obj->select_spiro("%%", "%%", "%%");
}
 $count = mysqli_num_rows($result_spiro);
