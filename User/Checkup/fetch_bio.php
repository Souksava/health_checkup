 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   ...
   $obj->select_biochemistry_limit("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%",$page);
}
else
{
   $obj->select_biochemistry_limit("%%", "%%", "%%", $page);
}
// $output .= '
// <style>
...
// </style>
// ';
if(mysqli_num_rows($result_biochemistry_limit) > 0)
{
 $output .= '
 <div class="table-responsive">
    <table class="table-bordered" style="width: 8000px;text-align: center;">
        <tr style="font-size: 18px;">
            <th style="width: 100px;">ເຄື່ອງມື</th>
            ...
            <th style="width: 587px;">Triglycerides(Trig)</th>
 ...
 ';
 $no_ = $rank;
 while($row = mysqli_fetch_array($result_biochemistry_limit))
 {
$no_ += 1;
  $output .= '
    <tr>
 ...
 mysqli_free_result($result_biochemistry_limit);  
...
{
   $obj->select_biochemistry("%%", "%%", "%%");
}
 $count = mysqli_num_rows($result_biochemistry);
 mysqli_free_result($result_biochemistry);  
 mysqli_next_result($conn);
 $a = ceil($count/100);
 if(isset($_POST['page'])){
    if($_POST['page'] > 1){
