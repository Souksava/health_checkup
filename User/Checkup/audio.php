<!-- audiogram ການກວດສະມັດຕະພາບການໄດ້ຍິນ -->
<?php
  $title = "ການກວດສະມັດຕະພາບການໄດ້ຍິນ";
  $path = "../../";
  $links = "../";
  $session_path = "../../";
  include ("../../header-footer/header.php");
?>
<form action="Audio" method="POST" id="form_upload" enctype="multipart/form-data">
...
<form action="export/export_audio.php" method="POST" id="form_export" target="_blank">
    <div class="row">
...
<form action="Audio" id="formDelete" method="POST" enctype="multipart/form-data">
 ...
 if(isset($_POST['btnDelete'])){
    if(isset($_POST["id_delete"])){
        foreach($_POST['id_delete'] as $id){
            $result_delete = mysqli_query($conn,"call del_audiogram('$id')");
        }
        if(!$result_delete){
            echo"<script>";
            echo"window.location.href='Audio?delete2=fail';";
            echo"</script>";
        }
        else{
            echo"<script>";
            echo"window.location.href='Audio?delete=success';";
            echo"</script>";
        }
    }
    else{
        echo"<script>";
        echo"window.location.href='Audio?del=null';";
        echo"</script>";
    }
 }
    include ("../../header-footer/footer.php");
    if(isset($_POST["file_upload"])){
        $obj->import_audio($_FILES["checkup_file"]["tmp_name"],$_POST["year"],$user_id);
    }
...
function checkInputs_form_upload() {
    ...
    if (datepicker2Value !== "" && checkup_fileValue !== "") {
        ...
        document.getElementById("form_upload").action = "Audio";
