...
?>
<form action="Urine" method="POST" id="form_upload" enctype="multipart/form-data">
    <div class="modal fade" id="exampleModalimpEmp" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">ນຳເຂົ້າຂໍ້ມູນການກວດປັດສະວະ</h5>
...
<form action="export/export_urine.php" method="POST" id="form_export" target="_blank">
    <div class="row">
...
<form action="Urine" id="formDelete" method="POST" enctype="multipart/form-data">
 ...
 if(isset($_POST['btnDelete'])){
    if(isset($_POST["id_delete"])){
        foreach($_POST['id_delete'] as $id){
            $result_delete = mysqli_query($conn,"call del_urinalvsis('$id')");
        }
        if(!$result_delete){
            echo"<script>";
            echo"window.location.href='Urine?delete2=fail';";
            echo"</script>";
        }
        else{
            echo"<script>";
            echo"window.location.href='Urine?delete=success';";
            echo"</script>";
        }
    }
    else{
        echo"<script>";
        echo"window.location.href='Urine?del=null';";
        echo"</script>";
    }
 }
       include ("../../header-footer/footer.php");
       if(isset($_POST["file_upload"])){
           $obj->import_urine($_FILES["checkup_file"]["tmp_name"],$_POST["year"],$user_id);
       }
...
function checkInputs_form_upload() {
    ...
    if (datepicker2Value !== "" && checkup_fileValue !== "") {
        ...
        document.getElementById("form_upload").action = "Urine";
