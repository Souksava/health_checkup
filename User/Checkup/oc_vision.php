<!-- oc_vision ການກວດສະມັດຕະພາບການເບິ່ງເຫັນ-->
<?php
  $title = "ການກວດສະມັດຕະພາບການເບິ່ງເຫັນ";
  $path = "../../";
  $links = "../";
  $session_path = "../../";
  include ("../../header-footer/header.php");
?>
<form action="Vision" method="POST" id="form_upload" enctype="multipart/form-data">
    <div class="modal fade" id="exampleModalimpEmp" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">ນຳເຂົ້າຂໍ້ມູນການກວດສະມັດຕະພາບການເບິ່ງເຫັນ</h5>
...
<form action="export/export_oc_vision.php" method="POST" id="form_export" target="_blank">
    <div class="row">
...
<form action="Vision" id="formDelete" method="POST" enctype="multipart/form-data">
 ...
 if(isset($_POST['btnDelete'])){
    if(isset($_POST["id_delete"])){
        foreach($_POST['id_delete'] as $id){
            $result_delete = mysqli_query($conn,"call del_oc_vision('$id')");
        }
        if(!$result_delete){
            echo"<script>";
            echo"window.location.href='Vision?delete2=fail';";
            echo"</script>";
        }
        else{
            echo"<script>";
            echo"window.location.href='Vision?delete=success';";
            echo"</script>";
        }
    }
    else{
        echo"<script>";
        echo"window.location.href='Vision?del=null';";
        echo"</script>";
    }
 }
...
function checkInputs_form_upload() {
    ...
    if (datepicker2Value !== "" && checkup_fileValue !== "") {
        ...
        document.getElementById("form_upload").action = "Vision";
