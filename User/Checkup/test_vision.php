<!-- Test Vision ທົດສອບສາຍຕາ(Test_vision)-->
<?php
  $title = "ທົດສອບສາຍຕາ";
  $path = "../../";
  $links = "../";
...
?>
<form action="Test-Vision" method="POST" id="form_upload" enctype="multipart/form-data">
    <div class="modal fade" id="exampleModalimpEmp" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">ນຳເຂົ້າຂໍ້ມູນການກວດກັ່ນຕອງມະເຮັງ</h5>
...
<form action="export/export_test_vision.php" method="POST" id="form_export" target="_blank">
    <div class="row">
...
<form action="Test-Vision" id="formDelete" method="POST" enctype="multipart/form-data">
 ...
 if(isset($_POST['btnDelete'])){
    if(isset($_POST["id_delete"])){
        foreach($_POST['id_delete'] as $id){
            $result_delete = mysqli_query($conn,"call del_test_vision('$id')");
        }
        if(!$result_delete){
            echo"<script>";
            echo"window.location.href='Test-Vision?delete2=fail';";
            echo"</script>";
        }
        else{
            echo"<script>";
            echo"window.location.href='Test-Vision?delete=success';";
            echo"</script>";
        }
    }
    else{
        echo"<script>";
        echo"window.location.href='Test-Vision?del=null';";
        echo"</script>";
    }
 }
    include ("../../header-footer/footer.php");
    if(isset($_POST["file_upload"])){
        $obj->import_test_vision($_FILES["checkup_file"]["tmp_name"],$_POST["year"],$user_id);
    }
...
function checkInputs_form_upload() {
    ...
    if (datepicker2Value !== "" && checkup_fileValue !== "") {
        ...
        document.getElementById("form_upload").action = "Test-Vision";
