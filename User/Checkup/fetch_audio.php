 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   ...
   $obj->select_audio_limit("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%",$page);
}
else
{
   $obj->select_audio_limit("%%", "%%", "%%", $page);
}
// $output .= '
// <style>
...
// </style>
// ';
if(mysqli_num_rows($result_audio_limit) > 0)
{
 $output .= '
 ...
 ';
 $no_ = $rank;
 while($row = mysqli_fetch_array($result_audio_limit))
 {
$no_ += 1;
  $output .= '
    <tr>
    <td><input type="checkbox" id="" name="id_delete[]" value="'.$row["audi_id"].'"></td>
        <td>'.$no_.'</td>
        <td>'.$row["barcode"].'</td>
        <td>'.$row["emp_id"].'</td>
        <td>'.$row["emp_name"].'</td>
 ...
 mysqli_free_result($result_audio_limit);  
 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   $obj->select_audio("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%");
}
else
{
   $obj->select_audio("%%", "%%", "%%");
}
 $count = mysqli_num_rows($result_audio);
