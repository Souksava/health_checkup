 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   ...
   $obj->select_heavy_metal_limit("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%",$page);
}
else
{
   $obj->select_heavy_metal_limit("%%", "%%", "%%", $page);
}
// $output .= '
// <style>
...
// </style>
// ';
if(mysqli_num_rows($result_heavy_metal_limit) > 0)
{
 $output .= '
 ...
 <table class="table-bordered" style="width: 6000px;text-align: center;">
   <tr style="font-size: 18px;">
         ...
         <th style="width: 87px;">Ether</th>
         <th style="width: 87px;">Ethyl Ethanoate</th>
         <th style="width: 87px;">Nickle in Blood</th>
         <th style="width: 87px;">Manganese in Blood</th>
         <th style="width: 87px;">Tin in Blood</th>
         <th style="width: 87px;">Mercury in Blood</th>
         <th style="width: 50px;">Mercury in Urine</th>
         <th style="width: 87px;">Butoxyacetic Acid in Urine</th>
         <th style="width: 50px;">Chromiun in Urine</th>
         <th style="width: 87px;">Iso Propyl Alcoho</th>
         <th style="width: 50px;">Silica</th>
         <th style="width: 150px;">Methy Hippuric acid in Urine</th>
         <th style="width: 150px;">Acetone in Urine</th>
         <th style="width: 87px;">Toluene in Urine</th>
         <th style="width: 250px;">Methyl lsobutyl Ketone in Urine</th>
         <th style="width: 87px;">Methanoi in Urine</th>
         <th style="width: 50px;">Phenolic Resin</th>
         <th style="width: 87px;">Conclusion</th>
         <th style="width: 50px;">Remark</th>
   </tr>
 ';
 $no_ = $rank;
 while($row = mysqli_fetch_array($result_heavy_metal_limit))
 {
$no_ += 1;
  $output .= '
    <tr>
    <td><input type="checkbox" id="" name="id_delete[]" value="'.$row["hea_id"].'"></td>
        <td>'.$no_.'</td>
        <td>'.$row["barcode"].'</td>
        <td>'.$row["emp_id"].'</td>
        <td>'.$row["emp_name"].'</td>
 ...
 mysqli_free_result($result_heavy_metal_limit);  
 ...
 if(isset($_POST["query"]) || isset($_POST["querys"]) || isset($_POST["year"]))
{
   $obj->select_heavy_metal("%".trim($_POST['query'])."%","%".trim($_POST['querys'])."%", "%".trim($_POST['year'])."%");
}
else
{
   $obj->select_heavy_metal("%%", "%%", "%%");
}
 $count = mysqli_num_rows($result_heavy_metal);
 mysqli_free_result($result_heavy_metal);  
 mysqli_next_result($conn);
 $a = ceil($count/100);
 if(isset($_POST['page'])){
    if($_POST['page'] > 1){
